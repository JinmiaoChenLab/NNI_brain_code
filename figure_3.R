...
rna.data = subset(rna.data, seurat_clusters != 13)
...
p = FeaturePlot(rna.data, c("oligo1", "macro1"), min.cutoff = "q05")

ggsave("../figure/oligo_macro_feature.pdf", p, width = 12, height = 6)

DimPlot(rna.data, label = T)
rna.data$cell.type = "Oligodendrocyte"
rna.data$cell.type[which(rna.data$seurat_clusters %in% c(7,10))] = "Microglia"
rna.data$cell.type[which(rna.data$seurat_clusters %in% c(8))] = "ME"
...
saveRDS(rna.data, "MDE.rds", compress = F)
########################################################################
...
rna.sub = subset(rna.data, cell.type == "Oligodendrocyte")
genes = c("HSPA1A","ADRB2","ATP6V0C","COL4A2","COLGALT2","FA2H","IKBIP","IQCB1","LEPROTL1","MALL","NKG7","OTUB1","SEC22A","TBK1","TRAF3","TRAF6","USP15","USP9X")
rna.sub = AddModuleScore(
  object = rna.sub,
  features = list(genes),
  name = "inter",
  seed = 1, ctrl = 50
...
p = VlnPlot(rna.sub, "USP15", group.by = "group", pt.size = 0, cols = c("#4D4D4D", "#8CDCDA", "#B1D877", "#F16A70"))+ NoLegend() +
  xlab("")
ggsave("../figure/USP15.pdf", width = 5, height = 4)

p = VlnPlot(rna.sub, "USP9X", group.by = "group", pt.size = 0, cols = c("#4D4D4D", "#8CDCDA", "#B1D877", "#F16A70"))+ NoLegend() +
  xlab("")
ggsave("../figure/USP9X.pdf", width = 5, height = 4)

p = VlnPlot(rna.sub, "HSPA1A", group.by = "group", pt.size = 0, cols = c("#4D4D4D", "#8CDCDA", "#B1D877", "#F16A70"))+ NoLegend() +
  xlab("")
ggsave("../figure/HSPA1A.pdf", width = 5, height = 4)
...
mm = FindMarkers(rna.data, ident.1 = c("DLB", "PDD", "ADD"), ident.2 = c("CTRL"), test.use = "MAST", latent.vars = "sex")
mm = mm[which(mm$p_val_adj < 0.01),]
mm$gene = rownames(mm)
...
p = VlnPlot(rna.data, "MSR1", group.by = "group", pt.size = 0, cols = c("#4D4D4D", "#8CDCDA", "#B1D877", "#F16A70"))+ NoLegend() +
  xlab("")
ggsave("../figure/MSR1.pdf", width = 5, height = 4)
